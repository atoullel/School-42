...
static	char	*ft_str_reverse(char *str)
{
	size_t	i;
	size_t	j;
	size_t	k;
...
static	void	ft_s_output(t_info info, int *total_printed, char *s)
{
	size_t len;
	int x;
...
static	void	ft_printdec(t_info info, int *total_printed, int n)
{
	char *result;
	char *tmp;
	int len;
	...
	if (len == 1 && result[0] == '0' && info.preci_bool == 1 && info.precision == 0)
	{
		while (info.width--)
		{
			write(1, " ", 1);
...
static	char	*ft_itoh(unsigned int nb)
{
...
static	void	ft_printhex(t_info info, int *total_printed, unsigned int n)
{
	char *result;
	int len;
	int x;
...
int	ft_printfV2(const char *format, ...)
{
	va_list	ap;
	size_t	i;
	int		total_printed;
